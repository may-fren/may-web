...
export default function Tests() {
  ...
  const canCreate = hasPermission('TEST_CREATE');
  const canUpdate = hasPermission('TEST_UPDATE');
  const canDelete = hasPermission('TEST_DELETE');
  ...
  const fetchData = useCallback(async (
    ...
    setLoading(true);
    try {
      const params: TestListParams = { page, size, sort };
      ...
      message.error('Testler yüklenirken hata oluştu');
    } finally {
      setLoading(false);
    }
  ...
  const openEditModal = (record: Test) => {
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true);
      if (editingRecord) {
        await updateTest(editingRecord.id, values as TestUpdateRequest);
        message.success('Test güncellendi');
      } else {
        await createTest(values as TestCreateRequest);
        message.success('Test oluşturuldu');
      }
      setModalOpen(false);
      form.resetFields();
      setEditingRecord(null);
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteTest(id);
      message.success('Test silindi');
      fetchData((pagination.current ?? 1) - 1, pagination.pageSize ?? 10);
    } catch {
      message.error('Test silinemedi');
    }
  };
  ...
  const columns = [
    { title: 'Test Adı', dataIndex: 'name', sorter: true, sortOrder: getSortOrder('name') },
    ...
    ...((canUpdate || canDelete) ? [{
      ...
      render: (_: unknown, record: Test) => (
        <Space>
          ...
          {canDelete && (
            <Popconfirm
              title="Bu testi silmek istediğinize emin misiniz?"
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Typography.Title level={3} style={{ margin: 0 }}>Testler</Typography.Title>
        {canCreate && (
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <Form form={filterForm} layout="vertical">
          <Row gutter={16}>
            <Col span={8}>
              <Form.Item name="name" label="Test Adı" style={{ marginBottom: 0 }}>
      ...
      <Modal
        title={editingRecord ? 'Test Düzenle' : 'Yeni Test'}
        open={modalOpen}
        onOk={handleSave}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="Test Adı" rules={[{ required: true, message: 'Zorunlu alan' }]}>
            <Input placeholder="Örn: Fren Performans Testi" />
          </Form.Item>
          <Form.Item name="description" label="Açıklama">
            <Input.TextArea rows={2} placeholder="Test açıklaması" />
          </Form.Item>
